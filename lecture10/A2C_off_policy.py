...
def A2C_learning(env, num_episodes=NUM_EPISODES):
    ...
    actor_beta = network.ActorNetwork(state_size, action_size)
    ...
    for episode_num in range(num_episodes):
        ...
        for _ in range(MAX_EPISODE_LENGTH):
            ...
            action_probs = actor_beta(state_tensor)
            ...
            importance_ratio = actor(state_tensor).squeeze(0)[action] / action_probs.squeeze(0)[action]

            # Critic update: Minimize squared TD error
            critic_loss = 0.5 *(importance_ratio.detach() * td_error).pow(2)
            critic_optimizer.zero_grad()
            critic_loss.backward()
            critic_optimizer.step()

            # Actor update: Policy gradient ∇logπ(a|s) * Q(s, a)
            log_pi = torch.log(actor(state_tensor).squeeze(0)[action] + 1e-8)  # Add small constant to avoid log(0)
            actor_loss = -log_pi * importance_ratio.detach() * td_error.detach()
...
if __name__ == "__main__":
    ...
    # Start training
    print("Starting A2C off-policy training...")
    trained_actor, trained_critic, rewards, lengths = A2C_learning(env, num_episodes=NUM_EPISODES)

    # Plot training curves
    print("Plotting training curves...")
    plot_training_history(rewards, lengths, save_path="images/A2C_off_policy_LunarLander_training_history.png")

    # Visualize trained policy
    print("Generating policy visualization GIF...")
    visualize_policy(trained_actor, env_name='LunarLander-v2', num_episodes=3, output_path='images/A2C_off_policy_LunarLander_trained_policy.gif')
