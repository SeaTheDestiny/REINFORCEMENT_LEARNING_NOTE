...
def update_parameter_and_policy(w, experience, env):
    """
    Use PyTorch automatic differentiation to compute gradients and manually update parameters
    """
    ...
    # 2. Compute target Q Target
    with torch.no_grad():
        if next_action is None: # Terminal
            q_target = reward
        else:
            feature_next_state = get_feature(next_state, m, n)
            q_next_state_all = torch.matmul(feature_next_state, w)
            q_target = reward + gamma * q_next_state_all[next_action]
...
def sarsa_learning(env, num_episodes=NUM_EPISODES):
    """
    Main SARSA learning loop with history tracking
    """
    ...
    for episode_num in range(num_episodes):
        ...
        for _ in range(MAX_EPISODE_LENGTH):
            ...
            if done:
                ...
                next_action = env.sample_action(next_state)
...
def plot_training_history(rewards, lengths, save_path="images/training_history.png"):
    """
    ...
    # If you want to limit y-axis range like your sample plot, uncomment the line below
    # plt.ylim(-1200, 100)
...
if __name__ == "__main__":
    ...
    trained_w, rewards, lengths = sarsa_learning(env, num_episodes=NUM_EPISODES)

    # Plot training curve (Total Reward & Episode Length)
    print("Plotting training process curve...")
    plot_training_history(rewards, lengths, save_path="images/training_history.png")
    ...
    gw.plot_policy_and_value(
        ...
        forbidden=env.forbidden,
        goal=env.goal,
        save_path="images/SARSA_FunctionApprox_policy_and_value.png"
    )
