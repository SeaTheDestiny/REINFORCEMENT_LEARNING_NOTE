...
def update_q_value_and_update_policy_from_q(q_values, experience, learning_rate=0.1):
    """
    ...
    if next_action is not None:
        q_target = reward + gamma * q_values[next_state_idx][next_action]
    else:
    ...
    return next_state, next_action, env.policy, q_values



def sarsa_learning(env, num_episodes=NUM_EPISODES):
    """
    Main SARSA learning loop.
    ...
    for episode_num in range(num_episodes):
        ...
        for _ in range(MAX_EPISODE_LENGTH):
            ...
            experience = (state, action, reward, next_state, None if done else env.sample_action(next_state))

            state, action, env.policy, q_values = update_q_value_and_update_policy_from_q(
                q_values, experience, learning_rate=0.1
            )
...
if __name__ == "__main__":
    ...
    q_values = sarsa_learning(env, num_episodes=NUM_EPISODES)
    ...
    gw.plot_policy_and_value(
        ...
        forbidden=env.forbidden,
        goal=env.goal,
        save_path="images/SARSA_policy_and_value.png"
    )
